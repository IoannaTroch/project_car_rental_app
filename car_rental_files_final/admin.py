...
def print_result(success, result):
    ...
    Handles 'None' results from Updates/Deletes gracefully.
    """
    if success:
        if result is None:
...
def run_admin_interface(model):
    ...
    while True:
        clear_screen()
        print("      ADMINISTRATIVE CONSOLE - V3.6 (FINAL)")
        print("=" * 60)
        ...
        if choice == '1': menu_customers(model)
        elif choice == '2': menu_cars(model)
        elif choice == '3': menu_reservations(model)
        elif choice == '4': menu_employees(model)
        elif choice == '5': menu_reports(model)
        elif choice == '6': menu_operations(model)
        ...
        if sel == '1':
            ...
            if success and rows:
                ...
                for r in rows:
                    ...
                    print(f"{cid:<5} {name:<25} {email:<30} {phone}")
            else:
            ...
            data = {'name': name, 'email': email, 'phone': phone, 'license': license_id, 'address': address, 'dob': dob}
            ...
            input("Press Enter...")
        elif sel == '5' or sel.lower() == 'q': return
...
def menu_cars(model):
    while True:
        ...
        if sel == '1':
            ...
            if success and rows:
                print(f"\n{'ID':<4} {'MODEL':<20} {'PLATE':<10} {'PRICE':<6} {'MIL':<6} {'STATUS'}")
                print("-" * 65)
                for r in rows:
                    status = "Active" if r.get('availability', 1) == 1 else "Retired"
                    cid = r.get('car_id', 'N/A')
                    brand = r.get('brand', '')
                    model_n = r.get('model', '')
                    plate = r.get('plate', r.get('plate_id', 'N/A'))
                    price = r.get('price_per_day', 'N/A')
                    mil = r.get('mileage', '0')
                    print(f"{cid:<4} {brand} {model_n:<15} {plate:<10} {price:<6} {mil:<6} {status}")
            else:
                print("No cars found.")
            input("\nPress Enter...")
            ...
            data = {'plate': plate, 'brand': brand, 'model': model_n, 'price': price, 'color': color, 'gear': gear, 'mil': mil, 'loc_id': loc_id, 'cat_id': cat_id}
        ...
        elif sel == '7' or sel.lower() == 'q': return
    ...
    while True:
        ...
        if sel == '1':
            ...
            if s and rows:
                ...
                print("-" * 80)
                for r in rows:
                    rid = r.get('reservation_id', 'N/A')
                    name = r.get('full_name', 'N/A')
                    car = f"{r.get('brand','')} {r.get('model','')}"
                    p_date = r.get('pick_up_date', '')
                    d_date = r.get('drop_off_date', '')
                    status = r.get('status', 'Confirmed') # Default if missing
                    print(f"#{rid:<4} {name:<20} {car:<20} {status:<12} {p_date} -> {d_date}")
            else:
                if not s:
                    print(f"\n DATABASE ERROR: {rows}")
                else:
            ...
            if s and rows:
                ...
                for r in rows:
                    ...
                    print(f"#{rid:<4} {name:<20} {car:<20} {status:<12} {p_date}")
            else:
                print("No active or future reservations found.")
            input("\nPress Enter...")

        elif sel == '3':
            cust_id = get_int("Customer ID"); car_id = get_int("Car ID")
            if not cust_id or not car_id: continue
            p_date = get_input("Pickup (YYYY-MM-DD)"); d_date = get_input("Dropoff (YYYY-MM-DD)")
            p_loc = get_int("Pickup Loc ID"); d_loc = get_int("Dropoff Loc ID")
            ins_id = get_int("Ins ID (0/1)"); cat_id = get_int("Cat ID")
            if all([p_date, d_date, p_loc, d_loc]):
                data = {'cust_id': cust_id, 'car_id': car_id, 'p_date': p_date, 'd_date': d_date, 'p_loc': p_loc, 'd_loc': d_loc, 'ins_id': ins_id, 'cat_id': cat_id}
        ...
        if sel == '1':
            ...
            if s and rows:
                ...
                for r in rows:
                    ...
                    email = r.get('email', 'N/A')
                    print(f"#{eid:<4} {name:<20} {email}")
            else: print("No employees found.")
            input("Press Enter...")

        elif sel == '2':
            # --- NEW FEATURE: WORK HISTORY ---
            ...
            if all(data.values()): print_result(*model.add_employee(data))
            input("Press Enter...")

        elif sel == '4':
            eid = get_int("Employee ID")
            if eid: print_result(*model.delete_employee(eid))
            input("Press Enter...")
        elif sel == '5' or sel.lower() == 'q': return
    ...
    while True:
        ...
        if sel == '1':
            ...
            if s and rows:
                ...
                print("-" * 50)
                for r in rows:
                    pid = r.get('payment_id', r.get('payment_number', 'N/A'))
                    amt = r.get('amount', r.get('total_amount', '0'))
                    name = r.get('full_name', 'N/A')
                    print(f"{pid:<10} {amt} EUR   {name}")
            else:
            ...
            input("Press Enter...")
        elif sel == '6' or sel.lower() == 'q': return
...
def menu_operations(model):
    while True:
        ...
        if sel == '1':
            ...
            if s and rows:
                for r in rows: print(f"LOG #{r['id']} | RES #{r['reservation_id']} | Status: {r.get('state', 'N/A')}")
            else: print("No Pickup logs found.")
            input("Press Enter...")
            ...
            if s and rows:
                for r in rows: print(f"LOG #{r['id']} | RES #{r['reservation_id']} | Status: {r.get('state', 'N/A')}")
            else: print("No Dropoff logs found.")
            input("Press Enter...")
            ...
            if all([res_id, emp_id, loc_id]):
                data = {'res_id': res_id, 'emp_id': emp_id, 'loc_id': loc_id, 'state': state, 'date': date_val}
