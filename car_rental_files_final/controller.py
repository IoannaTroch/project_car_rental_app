...
from datetime import datetime           # <--- REQUIRED for Date Logic
from difflib import get_close_matches   # <--- REQUIRED for Email Logic
...
class RentalController:
    def __init__(self):
        ...
        self.HIERARCHY = {1: ["City Car", "Compact Hatch"], 2: ["Crossover", "Full SUV"], 3: ["Luxury Compact", "Executive"], 4: ["MPV", "Passenger Van"]}
        self.SUBCAT_MAPPING = {"City Car": ["Yaris", "Panda", "Micra", "i10", "Aygo", "Picanto", "500"], "Compact Hatch": ["Clio", "Polo", "Corsa", "i20", "Fiesta", "Astra", "Tipo", "Series 1", "A3", "A-Class"], "Crossover": ["C-HR", "Vitara", "T-Roc", "Duster", "Captur", "Qashqai", "Tucson", "3008"], "Full SUV": ["X5", "Q8", "Tiguan", "S90"], "Luxury Compact": ["A-Class", "Series 1", "A3"], "Executive": ["Octavia", "S90", "Model 3", "C-Class", "Corolla"], "MPV": ["C4 Grand", "Zafira"], "Passenger Van": ["Vito", "Transporter", "Transit"]}
        self.INSURANCE_PLANS = [{"id": 0, "name": "Basic", "price": 0, "desc": "Standard Liability"}, {"id": 1, "name": "Full", "price": 15, "desc": "Zero Excess"}]
        
        self.show_role_selector()
        self.view.mainloop()

    def on_bank_response(self, client, userdata, msg):
        try: self.payment_status = json.loads(msg.payload.decode())
        except: pass

    def _load_init_data(self):
        try:
            s1, locs = self.model.get_locations(); s2, cats = self.model.get_categories()
    ...
    def show_role_selector(self):
        self.view.show_role_selection(admin_command=self.launch_admin_terminal, user_command=self.handle_user)
    def handle_user(self): self.view.show_main_screen(rent_command=self.goto_customer_details)
    ...
    def launch_admin_terminal(self):
        try:
            ...
            if self.console:
                sys.stdout = self.console; sys.stdin = self.console; sys.stderr = self.console
            
            def run_admin_thread():
                try: admin.run_admin_interface(self.model)
                except Exception as e: print(f"CRITICAL ERROR INSIDE THREAD: {e}")
                finally: self.view.after(0, self.view.destroy)
    ...
    def show_home(self): self.view.show_main_screen(rent_command=self.goto_customer_details)
    def goto_customer_details(self): self.view.show_customer_screen(submit_command=self.save_customer_and_next)
    ...
    def save_customer_and_next(self):
        ...
        # A. Check for typos in Major Providers (e.g., gmaik -> gmail)
        major_providers = [
            'gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 
            'icloud.com', 'live.com', 'msn.com', 'aol.com', 'yahoo.gr'
        ]
        
        # "cutoff=0.85" ensures high similarity to trigger "Did you mean?"
        matches = get_close_matches(domain_part, major_providers, n=1, cutoff=0.85)
        
        if matches and domain_part != matches[0]:
            # Found a typo! (e.g. user typed gmaik.com, we found gmail.com)
            self.view.show_error(f"Invalid email domain: '{domain_part}'.\nDid you mean '{matches[0]}'?")
            return

        # B. Check for common TLD typos (ending in .con, .cmo, etc)
        ...
        try:
            ...
            # Calculate Exact Age
            # (Subtract 1 from age if we haven't reached the birthday this year yet)
            age = today.year - dob.year - ((today.month, today.day) < (dob.month, dob.day))
            
            # STRICT CHECK: Must be 23 or older
            if age < 23:
            ...
            # Logic Integrity: License can't be issued before you were ~18
    ...
    def back_to_reservation(self):
        ...
        self.view.show_reservation_screen(search_command=self.perform_search, car_types=self.categories, locations=self.locations, time_slots=slots)

    def perform_search(self):
        c = self.view.combos; cal = self.view.calendar_selection
        if not cal['start'] or not cal['end']: self.view.show_error("Please select a date range!"); return
        self.current_booking['dates'] = {'start': cal['start'], 'end': cal['end'], 'start_time': c['pickup_time'].get(), 'end_time': c['dropoff_time'].get()}
        cat_id = self.cat_map.get(c['car_type'].get(), 1); loc_id = self.loc_map.get(c['pickup_loc'].get(), 1)
        self.current_booking['loc_id'] = loc_id
        success, cars = self.model.get_available_cars_for_booking(cat_id, loc_id)
        if not success: cars = []
        s_str = cal['start'].strftime("%Y-%m-%d"); e_str = cal['end'].strftime("%Y-%m-%d")
        ...
        self.current_cat_id = cat_id; self.refresh_subcategory_view()

    def refresh_subcategory_view(self):
        vars = self.view.filter_vars; filtered = self.search_results[:]
        if vars.get("automatic_only") and vars["automatic_only"].get(): filtered = [c for c in filtered if c['gearbox'] == 'Automatic']
        if vars.get("diesel_only") and vars["diesel_only"].get(): filtered = [c for c in filtered if c['fuel'] == 'Diesel']
        if vars.get("hybrid_only") and vars["hybrid_only"].get(): filtered = [c for c in filtered if c['fuel'] in ['Hybrid', 'Electric']]
        ...
        for sub in hierarchy:
            ...
            if best: display_data[sub] = {'available': True, 'min_price': best['price_per_day'], 'car': {'make': best['brand'], 'model': best['model'], 'trans': best['gearbox'], 'fuel': best['fuel'], 'seats': best['seats'], 'bags': best['bags']}}
            else: display_data[sub] = {'available': False, 'car': {}, 'min_price': 0}
        self.view.show_subcategory_screen(subcats_status=display_data, on_subcat_click=self.select_subcategory, on_filter_change=self.refresh_subcategory_view, on_home_click=self.back_to_reservation)

    def select_subcategory(self, subcat_name):
        filtered = self.search_results; models = self.SUBCAT_MAPPING.get(subcat_name, [])
        matches = [c for c in filtered if c['model'] in models or c['brand'] in models]
        if not matches: return
        target_car = min(matches, key=lambda x: x['price_per_day'])
        self.current_booking['car'] = target_car; self.current_booking['subcat_name'] = subcat_name
        self.show_summary_screen()

    def show_summary_screen(self):
        car = self.current_booking['car']
        summary = {'subcat': self.current_booking['subcat_name'], 'price': car['price_per_day'], 'trans': car['gearbox'], 'fuel': car['fuel'], 'seats': car['seats'], 'bags': car['bags']}
        self.view.show_booking_overview(summary=summary, insurance_options=self.INSURANCE_PLANS, late_policy="A €50 Security Deposit is charged upfront...", on_back_click=self.back_to_reservation, on_next_click=self.calculate_invoice, on_home_click=self.show_home)

    def calculate_invoice(self):
        d1 = self.current_booking['dates']['start']; d2 = self.current_booking['dates']['end']
        days = (d2 - d1).days; days = 1 if days < 1 else days
        daily_rate = self.current_booking['car']['price_per_day']; rental_total = days * daily_rate
        ins_id = self.view.insurance_var.get(); ins_plan = next((p for p in self.INSURANCE_PLANS if p['id'] == ins_id), self.INSURANCE_PLANS[0])
        grand_total = rental_total + (days * ins_plan['price']) + 50.0
        inv_data = {'category': self.current_booking['subcat_name'], 'days': days, 'rental_total': rental_total, 'daily_rate': daily_rate, 'ins_name': ins_plan['name'], 'ins_total': (days * ins_plan['price']), 'deposit': 50.0, 'grand_total': grand_total}
    ...
    def initiate_payment(self, inv_data):
        ...
        if MQTT_AVAILABLE:
            ...
            try: self.app_mqtt.publish(TOPIC_REQUEST, json.dumps(req))
            except: 
    ...
    def finalize_booking(self, inv_data):
        try:
            d = self.current_booking; cust_data = d['customer']
            ...
            if s: self.model.add_payment(res_id, inv_data['grand_total'], cust_id)
            
            # 6. SHOW CONFIRMATION POPUP (BLOCKING)
            # The code stops here until you click "OK"
            ...
            # 7. RETURN TO START (Runs only after popup is closed)
            self.current_booking = {} 
